...
func (d Engine) ReadSchema(schemaName string, db *sql.DB) (*gendriver.SchemaData, error) {
	...
	for _, field := range fields {
		...
		case "char":
			var maxLength int32
			if field.CharacterMaximumLength.Valid {
				maxLength = field.CharacterMaximumLength.Int32
			}
			colsData[field.Ordinal].Type = gendriver.NewTChar(maxLength)
		case "varchar":
			var maxLength int32
			if field.CharacterMaximumLength.Valid {
				maxLength = field.CharacterMaximumLength.Int32
			}
			colsData[field.Ordinal].Type = gendriver.NewTVarChar(maxLength)
		case "text":
